...
data Sluice = Sluice
  { _eventSrc :: IO WrappedKeyEvent      -- ^ Where we get our 'KeyEvent's from
  , _blocked  :: IORef Int        -- ^ How many locks have been applied to the sluice
  , _blockBuf :: IORef [WrappedKeyEvent] -- ^ Internal buffer to store events while closed
  }
makeLenses ''Sluice

-- | Create a new 'Sluice' environment
mkSluice' :: MonadUnliftIO m => m WrappedKeyEvent -> m Sluice
...
mkSluice :: MonadUnliftIO m => m WrappedKeyEvent -> ContT r m Sluice
...
-- "KMonad.App" module.)
unblock :: HasLogFunc e => Sluice -> RIO e [WrappedKeyEvent]
unblock s = do
...
step :: HasLogFunc e => Sluice -> RIO e (Maybe WrappedKeyEvent)
step s = do
  e <- liftIO $ s^.eventSrc
  readIORef (s^.blocked) >>= \case
    0 -> pure $ Just e
