...
-- | A variant of `around` which releases it's outter button when another
-- key is pressed.
aroundOnly ::
     Button -- ^ The outer 'Button'
  -> Button -- ^ The inner 'Button'
...
aroundWhenAlone outer inner = onPress $ do
 ...
 where
  ...
  go isMyRelease pressed = hookF InputHook $ \e ->
    if
      | isMyRelease e -> do
        runAction $ inner^.releaseAction
      ...
      | otherwise -> do
        ...
        await isRelease $ \_ -> go isMyRelease pressed' $> NoCatch
        pure Catch
