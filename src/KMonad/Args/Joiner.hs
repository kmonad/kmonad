...
joinLayer als ns srcs l@(DefLayer n lss) = do
  let bs = lss^..each._LButton
  assocSrc <- getAssocSrc l
  implArnd' <- getImplArnd l
  ...
  maybe id (local . set implArnd) implArnd' $
    (n,) <$> foldM f [] (zip src bs)

getAssocSrc :: DefLayer -> J (Maybe Text)
getAssocSrc (DefLayer n lss) = case onlyOne $ lss^..each._LSrcName of
  Right x        -> pure $ Just x
  Left None      -> pure Nothing
  Left Duplicate -> throwError $ DuplicateLayerSetting n "source"

getImplArnd :: DefLayer -> J (Maybe ImplArnd)
getImplArnd (DefLayer n lss) = case onlyOne $ lss^..each._LImplArnd of
  Right x        -> pure $ Just x
  Left None      -> pure Nothing
